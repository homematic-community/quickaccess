			...
			array set res [rega_script {
				...
				if ((s_sys_pin != "") && (s_sys_pin.ToInteger() != (o_sys_pin.Timestamp().ToInteger() - "} $pin {".ToInteger()))) {
					...
					if (s_list.Substr (0,2) == 'HM') {
						...
						WriteXML (s_list);
						WriteLine ('</h1>');
						WriteLine ('<a href="javascript:jumpto(\'hardware.cgi\');"><div onclick="this.className=\'active\';" class="button">zurück</div></a>');
					...
					WriteLine ('<a href="javascript:jumpto(\'help/channels.htm\');"><div class="buttonhelp">Hilfe</div></a>');
					WriteLine ("");
					...
					foreach (s_object_id, s_list_enum) {
						...
						if (i_temp >= 0) {
							WriteXML (o_object.Name().Substr (0, i_temp) # '-' # o_object.Name().Substr(i_temp + s_listname.Length(), o_object.Name().Length() - i_temp));
						} else {
						...
						foreach (s_dp, o_object.DPs().EnumUsedIDs() # '\tCONFIG_PENDING\tLOWBAT\tSTICKY_UNREACH\tUNREACH') {
							...
							while (s_button_labels.StrValueByIndex (":", i_button_index) != "") {
								...
								if ((s_button_type == "selector") || (s_button_type == "") ||
									((s_button_type == "button") && (s_button_action != o_dp.Value())) ||
									((s_button_type == "status") || (s_button_type == "info") || (s_button_type == "error") || (s_button_type == "warning")) && (i_button_index == o_dp.Value().ToInteger())) {
									i_temp = 1;
									while (s_button_label.StrValueByIndex ('<', i_temp) != "") {
										i_temp = i_temp + 1;
									}
									if ((s_button_type == "button") || (s_button_type == "selector")) {
									...
									if ((s_button_type == "error") || (s_button_type == "status") || (s_button_type == "warning")) {
										Write ('<div class="' # s_button_type # i_button_index # '">');
									}
									if (s_button_type == "button") {
										Write ('<div onclick="this.className=\'active\';" class="button' # o_dp.Value() # '">');
									}
									if (s_button_type == "selector") {
										Write ('<div onclick="this.className=\'active\';" class="button' # (r_temp == s_button_action.ToFloat()) # '">');
									}
